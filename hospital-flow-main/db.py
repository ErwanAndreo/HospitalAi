    ...
    def get_device_maintenance_urgencies(self) -> List[Dict]:
        ...
        try:
            ...
            with open(safe_log, 'a') as f:
                f.write(json.dumps({'sessionId': 'debug-session', 'runId': 'run1', 'hypothesisId': 'A', 'location': 'db.py:1802', 'message': 'get_device_maintenance_urgencies called', 'data': {'self_type': str(type(self)), 'log_used': safe_log}, 'timestamp': int(__import__('time').time() * 1000)}) + '\n')
        except:
            try:
                ...
                with open(log_path, 'a') as f:
                    f.write(json.dumps({'sessionId': 'debug-session', 'runId': 'run1', 'hypothesisId': 'A,E', 'location': 'db.py:1811', 'message': 'get_device_maintenance_urgencies log fallback', 'data': {'self_type': str(type(self))}, 'timestamp': int(__import__('time').time() * 1000)}) + '\n')
            except:
                pass
        # #endregion
        conn = self.get_connection()
